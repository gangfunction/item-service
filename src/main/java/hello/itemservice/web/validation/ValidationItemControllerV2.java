...
public class ValidationItemControllerV2 {
    ...
    public String addItem(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
        ...
        if (item.getPrice() == null || item.getPrice() < 1000 || item.getPrice() > 10000000) {
            bindingResult.addError(new FieldError("item", "price", "가격은 1,000,000원 이하여야합니다."));
        }
        if (item.getQuantity() == null || item.getQuantity() >= 9999) {
            bindingResult.addError(new FieldError("item", "quantity", "최소수량은 9999개 이하여야합니다."));
        }
        if (item.getPrice() != null && item.getQuantity() != null) {
            int resultPrice = item.getPrice() * item.getQuantity();
            if (resultPrice < 100000) {
                bindingResult.addError(new ObjectError("item", "가격 * 수량은 100000이하여야합니다."));
            }
        }
        if (!errors.isEmpty()) {
